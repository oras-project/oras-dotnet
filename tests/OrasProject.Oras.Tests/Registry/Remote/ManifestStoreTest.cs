...
public class ManifestStoreTest
{
    ...
    [Fact]
    public async Task ManifestStore_PushAsyncImageManifestWithSubjectAndNullConfig_DoesNotThrow()
    {
        // A non-conformant image manifest may carry "config": null. When the referrers API
        // is not supported, the tag-schema fallback derives the referrer artifactType from
        // the config media type; a null config must not throw there.
        var manifest = new Manifest
        {
            SchemaVersion = 2,
            MediaType = MediaType.ImageManifest,
            Config = null!,                 // "config": null on the wire
            Layers = [RandomDescriptor()],
            Subject = RandomDescriptor(),
        };
        ...
        // The referrer recorded in the tag-schema index is the manifest descriptor with an
        // artifactType derived from the (null) config => null, i.e. only mediaType/digest/size.
        var expectedReferrer = new Descriptor
        {
            MediaType = MediaType.ImageManifest,
            Digest = manifestDesc.Digest,
            Size = manifestDesc.Size,
        };
        var (expectedIndexReferrersDesc, _) = Index.GenerateIndex(new List<Descriptor> { expectedReferrer });

        var referrersTag = Referrers.BuildReferrersTag(manifest.Subject);
        byte[]? receivedManifestContent = null;
        byte[]? receivedIndexReferrersContent = null;
        ...
        {
            var response = new HttpResponseMessage { RequestMessage = req };
            if (req.Method == HttpMethod.Put && (
                    ...
                    req.RequestUri?.AbsolutePath == $"/v2/test/manifests/{referrersTag}"))
            {
                ...
                if (req.RequestUri.AbsolutePath == $"/v2/test/manifests/{manifestDesc.Digest}")
                    response.Headers.Add(_dockerContentDigestHeader, [manifestDesc.Digest]);
                else response.Headers.Add(_dockerContentDigestHeader, [expectedIndexReferrersDesc.Digest]);
                response.StatusCode = HttpStatusCode.Created;
                return response;
            }
        ...
        // Must not throw NullReferenceException from the null config.
        await store.PushAsync(manifestDesc, new MemoryStream(manifestBytes), cancellationToken);

        Assert.Equal(Referrers.ReferrersState.NotSupported, repo.ReferrersState);
        Assert.Equal(manifestBytes, receivedManifestContent);
        Assert.NotNull(receivedIndexReferrersContent);
    }
