...
public class ClientTest
{
    ...
    [Fact]
    public async Task SendAsync_BearerChallengeWithoutServiceParameter_Succeeds()
    {
        // Arrange
        var host = "example.com";
        var realm = "https://auth.example.com";
        string[] scopes = ["repository:repo1:pull"]; // minimal scope
        var refreshToken = "refresh_token";
        var expectedAccessToken = "access_token_no_service";

        async Task<HttpResponseMessage> Mock(HttpRequestMessage req, CancellationToken ct)
        {
            if (req.Method == HttpMethod.Post && req.RequestUri?.AbsoluteUri.TrimEnd('/') == realm.TrimEnd('/'))
            {
                // OAuth2 token request: since service is optional and missing in challenge, it should NOT be sent in form
                var formData = await req.Content!.ReadAsStringAsync(ct);
                var formValues = System.Web.HttpUtility.ParseQueryString(formData);
                if (formValues["grant_type"] == "refresh_token" && formValues["refresh_token"] == refreshToken && formValues["service"] == null && formValues["scope"] == string.Join(" ", scopes))
                {
            ...
            if (req.Method == HttpMethod.Get && req.RequestUri?.Host == host)
            {
                if (req.Headers.Authorization == null || req.Headers.Authorization.Scheme != "Bearer" || req.Headers.Authorization.Parameter != expectedAccessToken)
                {
                    return new HttpResponseMessage(HttpStatusCode.Unauthorized)
                    {
                        RequestMessage = req,
                        Headers =
                        {
                            WwwAuthenticate =
                            {
                                // Bearer challenge WITHOUT service param
                                new AuthenticationHeaderValue("Bearer", $"realm=\"{realm}\",scope=\"{string.Join(" ", scopes)}\"")
                            }
                        }
                    };
                }
        ...
        var handler = CustomHandler(Mock);
        var client = new Client(new HttpClient(handler.Object), credentialProvider.Object);
        var request = new HttpRequestMessage(HttpMethod.Get, $"https://{host}");
        ...
        handler.Protected().Verify(
            "SendAsync",
            Times.AtLeastOnce(),
            ItExpr.Is<HttpRequestMessage>(r => r.RequestUri != null && r.RequestUri.Host == host && r.Headers.Authorization != null && r.Headers.Authorization.Scheme == "Bearer" && r.Headers.Authorization.Parameter == expectedAccessToken),
            ItExpr.IsAny<CancellationToken>());
    }
}
