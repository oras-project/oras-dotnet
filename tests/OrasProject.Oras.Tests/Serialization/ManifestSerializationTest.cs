...
public partial class ManifestSerializationTest
{
    ...
    public void RoundTrip_PreservesWireFormat(string json, bool isIndex)
    {
        ...
        if (isIndex)
        {
            var idx2 = OciJsonSerializer.Deserialize<OciIndex>(reserialized)!;
            reserialized2 = OciJsonSerializer.SerializeToUtf8Bytes(idx2);
        }
        else
        {
            var m2 = OciJsonSerializer.Deserialize<Manifest>(reserialized)!;
            reserialized2 = OciJsonSerializer.SerializeToUtf8Bytes(m2);
        }
    ...
    [Theory]
    [MemberData(nameof(AllManifestRoundTripFixtures))]
    public void Deserialize_IgnoresUnknownFields(string json, bool isIndex)
    {
    ...
    /// </summary>
    public static IEnumerable<object[]> AllManifestRoundTripFixtures()
    {
        // OCI Manifests (isIndex = false)
        yield return new object[] { SpecImageManifestJson, false };
        yield return new object[] { MinimalManifestJson, false };
        yield return new object[] { FullManifestJson, false };
        yield return new object[] { LayerAnnotationManifestJson, false };

        // Docker Manifest (isIndex = false)
        yield return new object[] { DockerManifestJson, false };

        // Artifacts (isIndex = false)
        yield return new object[] { MinimalArtifactJson, false };
        yield return new object[] { ArtifactWithConfigJson, false };
        yield return new object[] { ArtifactPlusSignJson, false };

        // Indexes (isIndex = true)
        yield return new object[] { MinimalIndexJson, true };
        yield return new object[] { FullIndexJson, true };
        yield return new object[] { SpecMultiPlatformIndexJson, true };
        yield return new object[] { NestedIndexJson, true };

        // Docker manifest list (deserialized as Index)
        yield return new object[] { DockerManifestListJson, true };
    }
}
