...
class AzureCredentialProvider(string Host) : ICredentialProvider
{
    ...
    private ContainerRegistryClient AzureCRClient { get; set; } = new ContainerRegistryClient(new Uri($"https://{Host}"));

    private async Task<string> GetAadTokenAsync(CancellationToken cancellationToken)
    {
        if (string.IsNullOrEmpty(_aadToken))
        {
    ...
    public async Task<Credential> ResolveCredentialAsync(string hostname, CancellationToken cancellationToken)
    {
        ...
        if (!_credential.IsEmpty() && DateTime.UtcNow < TokenExpiry)
        {
            return _credential;
        }

        var aadToken = await GetAadTokenAsync(cancellationToken).ConfigureAwait(false);
        var response = AzureCRClient.ExchangeAadAccessTokenForAcrRefreshToken(hostname, null, null, aadToken, cancellationToken);
